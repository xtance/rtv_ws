...
{
	if(CS_GetTeamScore(2) + CS_GetTeamScore(3) < ROUNDS_MIN)
		PrintToChat(iClient, " \x06>>\x01 Надо сыграть хотя бы \x06%i раундов!", ROUNDS_MIN);
	else
	{
		if(bVoting)
			PrintToChat(iClient, " \x06>>\x01 Сейчас голосовать нельзя.");
		else if(bChange[iClient])
			PrintToChat(iClient, " \x06>>\x01 Ты уже голосовал за смену карты!");
		else
		{
			...
			if(f >= PERCENT)
			{
				...
				PrintToChatAll(" \x06>>\x01 %N хочет сменить карту -> \x06/rtv\x01 [%.0f%% из %.0f%%]", iClient, f, PERCENT);
		}
	}
